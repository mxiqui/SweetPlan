...
const PlanificadorViajeAlternativo = () => {
  ...
  const [selectedVueloAlternativo, setSelectedVueloAlternativo] = useState(null);
  ...
  const confirmarSeleccion = async () => {
    ...
    console.log(selectedVueloAlternativo)
  
    try {
      if(selectedVueloAlternativo ==null){  
      ...
      console.log('✅ Oferta enviada correctamente:', res.data);
        ...
        const resultadoFinalAlt = {
        datosBusqueda: formData,
        vueloSeleccionado: selectedVuelo,
        vueloSeleccionadoAlternativo: selectedVueloAlternativo,
        alojamientoSeleccionado: selectedAlojamiento,
    };
              const res = await axios.post('http://localhost:8080'+'/crearOfertaVariosVuelos', resultadoFinalAlt);
      console.log('✅ Oferta enviada correctamente:', res.data);
...
{Array.isArray(vuelos) && vuelos.length > 0 && (
        ...
        <>
          <h2>Vuelos Alternativos</h2>
          <div style={{ display: 'flex', gap: '20px', flexWrap: 'wrap' }}>
            {vuelos.map((vuelo, idx) => (
              <div
                key={idx}
                onClick={() => setSelectedVueloAlternativo(vuelo)}
                style={{
                  border: selectedVueloAlternativo === vuelo ? '3px solid green' : '1px solid gray',
                  padding: '10px',
                  cursor: 'pointer',
                  width: '300px',
                }}
...
export default PlanificadorViajeAlternativo;
